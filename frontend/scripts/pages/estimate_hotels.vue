  ...
  <div>
    ...
    <b-tabs>
      ...
      <b-tab :active="(project.people||[]).length>0 && (project.hotels||[]).length==0">
        ...
        <b-button @click="addFromPeople()" variant="primary">Add hotels from people <b-spinner v-if="loading_add_from_people" small type="grow" /></b-button>
...
export default {
  ...
  methods: {
    addFromPeople() {
      ...
      this.$http.post("/api/project/"+this.project.id+"/add_hotels_from_people", options).then((response) => {
