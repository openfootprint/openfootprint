  ...
  <div>
    ...
    <b-tabs>
      ...
      <b-tab :active="(project.people || []).length > 0 && (project.meals || []).length == 0">
        ...
        <b-form-group label="Grams per person per day" label-for="meals_from_people_massperday">
          <b-form-input id="meals_from_people_massperday" v-model="meals_from_people_massperday" />
        </b-form-group>

        <b-button variant="primary" @click="addFromPeople()">
          Add meals from people
...
export default {
  ...
  methods: {
    addFromPeople() {
      ...
      this.$http.post(this.project_api_root + "/add_meals_from_people", options).then(() => {
